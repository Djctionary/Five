...
/** Keeps a database failure readable instead of an opaque server exception. */
function dbFail(error: unknown): ActionResult {
  const { code, message } = describeDbError(error);
  if (code === "42P01") {
    return { ok: false, error: "数据库还没有建表，请先访问 /api/init 建表" };
  }
  // No SQLSTATE means it is not a database error, for example a missing
  // environment variable, so the message stands on its own.
  if (!code) return { ok: false, error: message };
  return { ok: false, error: `数据库出错 ${code}: ${message}` };
}
